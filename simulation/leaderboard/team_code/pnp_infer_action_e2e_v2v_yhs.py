...
class PnP_infer():
	...
	def get_action_from_list_multi_vehicle(self, car_data_raw, rsu_data_raw, step, timestamp):
		control_all = []
		infra_rsu_data_raw = rsu_data_raw[self.ego_vehicles_num:] if len(rsu_data_raw) > self.ego_vehicles_num else []
			...
			if car_data_raw[ego_id] is not None:
				...
				if len(rsu_data_raw) > 0:
					if rsu_data_raw[ego_id] is not None:
						...
						pass
				new_rsu_data_raw.extend([r for r in infra_rsu_data_raw if r is not None])
	...
	def get_action_from_list_inter(self, ego_id, car_data_raw, rsu_data_raw, step, timestamp):
		...
		try:
			...
			print(f"[yhs-viz] visualization failed at ego={ego_id}, step={step}: {type(exc).__name__}: {exc}; continuing eval")
		if lidar_3d is None:
			lidar_3d = np.zeros((1500, 2250, 3), dtype=np.uint8)
		if lidar_bev is None:
			lidar_bev = np.zeros((1500, 1500, 3), dtype=np.uint8)
...
def visualize(infer_result, pcd, pc_range, save_path, method='3d', left_hand=False, measurements=None, lidar_len=None, predicted_waypoints=None, target_point=None):
		...
		if pred_box_tensor is not None:
			if len(pred_box_tensor) > 0:
				...
				filte_array = pred_box_tensor.cpu().numpy()
		...
		if method == 'bev':
			...
			if measurements is not None and "lidar_pose_z" in measurements:
				pcd_np[:,2] -= float(measurements["lidar_pose_z"])
			pcd_np[:,1] *= -1
